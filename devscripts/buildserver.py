...
class GITInfoBuilder(object):
    def __init__(self, **kwargs):
        try:
            ...
            raise BuildError(f'Missing mandatory parameter "{e.args[0]}"')
...
class YoutubeDLBuilder(object):
    ...
    def __init__(self, **kwargs):
        if self.repoName != 'youtube-dl':
            raise BuildError(f'Invalid repository "{self.repoName}"')
        if self.user not in self.authorizedUsers:
            raise HTTPError(f'Unauthorized user "{self.user}"', 401)
...
class CleanupTempDir(object):
    def build(self):
        try:
            ...
            print(f'WARNING deleting "{self.basePath}": {e}')
    ...
    def do_GET(self):
        ...
        if path:
            ...
            if action in self.actionDict:
                ...
                self.send_response(500, f'Unknown build method "{action}"')
        else:
